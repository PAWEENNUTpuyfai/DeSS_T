...
func CallPythonAPIPOWER(number int) (int, error) {
    ...
    base := getPythonServiceBaseURL()
    resp, err := http.Post(base+"/api/power", "application/json", bytes.NewBuffer(data))
    if err != nil {
        return 0, err
    }
...
func CallPythonDistributionFit(data interface{}) (map[string]interface{}, error) {
    ...
    base := getPythonServiceBaseURL()
    resp, err := http.Post(base+"/api/distribution_fit", "application/json", bytes.NewBuffer(payload))
    if err != nil {
        return nil, err
    }
    defer resp.Body.Close()
