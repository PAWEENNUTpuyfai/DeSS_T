...
export default function ConfigurationDetailPage() {
  ...
  // Sort station pairs based on column selection
  const stationPairs = [...rawStationPairs].sort((a, b) => {
    if (!stationPairSort || !selectedStationId) return 0;
    
    if (stationPairSort === 'from') {
      // Pairs where selected station is FstStation come first
      const aIsFrom = a.FstStation === selectedStationId ? 1 : 0;
      const bIsFrom = b.FstStation === selectedStationId ? 1 : 0;
      return bIsFrom - aIsFrom;
    } else {
      // Pairs where selected station is SndStation come first
      const aIsTo = a.SndStation === selectedStationId ? 1 : 0;
      const bIsTo = b.SndStation === selectedStationId ? 1 : 0;
      return bIsTo - aIsTo;
    }
  });
  ...
  const filteredMapStations = stations.filter(s => {
    ...
    const matchSearch = stationSearch.trim() === '' ||
                  ...
                  {([
                    ...
                    <button
                      ...
                      style={{
                        ...
                        boxShadow: stationFilter === key ? '0 4px 10px rgba(129,6,158,0.22)' : 'none',
                      }}
                    >
                      {dot && <span className="w-2 h-2 rounded-full flex-shrink-0" style={{ background: stationFilter === key ? 'rgba(255,255,255,0.75)' : (dot ?? '') }} />}
                      {label}
                    </button>
                  ))}
                </div>
            ...
            <div className="flex-1 h-full pr-1 overflow-y-auto flex flex-col gap-3 pb-4">
              ...
              {selectedStation1 ? (
                <div className="bg-white rounded-2xl shadow-md border border-gray-200 flex-shrink-0">
                  <div className="flex items-center gap-2 px-4 pt-3 pb-2 border-b border-gray-100">
                    ...
                    <h3 className="text-sm font-bold text-gray-800">Station Details</h3>
                  </div>
                  <div className="px-4 py-3 space-y-1">
                    <p className="font-semibold text-gray-800">{selectedStation1.name}</p>
                    <p className="text-xs text-gray-500">ID: {selectedStation1.station_detail_id}</p>
                    <p className="text-xs text-gray-500">
                      Lat: {selectedStation1.lat?.toFixed(6)}, Lon: {selectedStation1.lon?.toFixed(6)}
                    </p>
                  </div>
                </div>
              ) : (
                <div className="bg-white rounded-2xl p-4 shadow-sm border border-dashed border-gray-200 flex-shrink-0">
                  <p className="text-gray-400 text-center text-sm">Click a station to view details</p>
                </div>
              )}
              ...
              <div className="bg-white rounded-2xl shadow-md border border-gray-200 flex-1 overflow-hidden flex flex-col">
                <div className="flex items-center gap-2 px-4 pt-3 pb-2 border-b border-gray-100 flex-shrink-0">
                  <span className="w-1 h-5 bg-[#81069e] rounded-full flex-shrink-0" />
                  <h3 className="text-sm font-bold text-gray-800">Distribution Data</h3>
                </div>
                <div className="overflow-y-auto flex-1 px-4 py-3">
                  {(filteredAlighting1.length > 0 || filteredInterarrival1.length > 0) ? (
                    <div className="space-y-3">
                      {(() => {
                        ...
                        return Array.from(timePeriods).sort().map((timePeriod) => {
                          ...
                          return (
                            <div key={timePeriod} className="border-b border-gray-100 pb-3 last:border-b-0">
                              {/* ── ③ Time Period Chip ── */}
                                ...
                                <table className="w-full text-xs border-separate border-spacing-0">
                                  <thead>
                                    <tr className="bg-[#f3e5f5]">
                                      <th className="text-left px-2 py-1.5 text-[#81069e] font-semibold rounded-tl-xl">Alighting Dist</th>
                                      <th className="text-left px-2 py-1.5 text-[#81069e] font-semibold">Args</th>
                                      <th className="text-left px-2 py-1.5 text-[#81069e] font-semibold">Interarrival Dist</th>
                                      <th className="text-left px-2 py-1.5 text-[#81069e] font-semibold rounded-tr-xl">Args</th>
                                    </tr>
                                  </thead>
                                  <tbody>
                                    <tr className="hover:bg-gray-50 transition-colors">
                                      <td className="px-2 py-1.5 text-gray-700 rounded-bl-xl">{alighting?.distribution || "N/A"}</td>
                                      <td className="px-2 py-1.5 font-mono text-gray-600">{alighting?.argument_list || "N/A"}</td>
                                      <td className="px-2 py-1.5 text-gray-700">{interarrival?.distribution || "N/A"}</td>
                                      <td className="px-2 py-1.5 font-mono text-gray-600 rounded-br-xl">{interarrival?.argument_list || "N/A"}</td>
                                    </tr>
                                  </tbody>
                                </table>
                                </div>
                    ...
                    <p className="text-gray-400 text-sm text-center py-4">No data for selected station</p>
                  )}
                </div>
              </div>
            </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[100000]">
          <div className="bg-white rounded-[40px] p-8 max-w-md w-full mx-4 border-2">
            ...
            <p className="text-gray-600 mb-6">
              Are you sure you want to delete "{configurationName}"? This action cannot be undone.
            </p>
